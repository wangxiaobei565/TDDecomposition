...
class A2C_WAmodel_WC(BaseRLAgent):
    @staticmethod
...
#         omiga = (self.alpha * reward + (1-self.alpha)) / (self.alpha * torch.sum(reward,dim=-1).view(-1,1) + (1-self.alpha)* reward.shape[1])
        omiga_c = (self.alpha * reward + (1-self.alpha)) / (self.alpha * torch.sum(reward,dim=-1).view(-1,1) + (1-self.alpha)* reward.shape[1])
...
#         import pdb
#         pdb.set_trace()
        Q_S = reward + self.gamma * omiga_c * (~done_mask * V_S_prime).view(-1,1)
