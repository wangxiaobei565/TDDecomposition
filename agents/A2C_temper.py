...
#         return episode_report, train_report
    ...
    def step_train(self):
        ...
        observation, policy_output, reward, done_mask, next_observation, step = self.facade.sample_buffer(self.batch_size)
#         reward = torch.FloatTensor(reward)
#         done_mask = torch.FloatTensor(done_mask)
        
        critic_loss, actor_loss, entropy_loss, advantage = self.get_a2c_loss(observation, policy_output, reward, done_mask, next_observation, step)
    ...
    def get_a2c_loss(self, observation, policy_output, reward, done_mask, next_observation, step,
...
#         V_S_prime = self.critic({'state_emb': S_prime})['v'].detach()
        ...
        Q_S = reward + (self.gamma-0.04*step) * (~done_mask * V_S_prime)
