...
class A2C_WAWC_new_alpha(BaseRLAgent):
    @staticmethod
    def parse_model_args(parser):
        ...
        parser.add_argument('--alpha_c', type=float, default=0.5, 
                            help='weighted reward')
...
#         V_S_prime = self.critic({'state_emb': S_prime})['v'].detach()
        omiga_a = (self.alpha_a * reward + (1-self.alpha_a)) / (self.alpha_a * torch.sum(reward,dim=-1).view(-1,1) + (1-self.alpha_a)* reward.shape[1])
        omiga_c = (self.alpha_c * reward + (1-self.alpha_c)) / (self.alpha_c * torch.sum(reward,dim=-1).view(-1,1) + (1-self.alpha_c)* reward.shape[1])
        
        Q_S = reward + self.gamma  * (~done_mask * V_S_prime).view(-1,1)/ reward.shape[1]
#         import pdb
#         pdb.set_trace()
        advantage = torch.clamp(((reward + self.gamma * omiga_a * (~done_mask * V_S_prime).view(-1,1)) - V_S/ reward.shape[1]).detach(), -1, 1) # (B,)

        # Compute critic loss
        loss_func = nn.MSELoss(reduce=False)
        value_loss = (omiga_c*reward.shape[1]*F.mse_loss(V_S.squeeze(), Q_S.sum(-1))).mean()
