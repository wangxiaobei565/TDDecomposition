...
#         V_S_prime = self.critic({'state_emb': S_prime})['v'].detach()
        ...
        Q_S = reward * reward.shape[1] + self.gamma * (~done_mask * V_S_prime).view(-1,1)
        
        advantage = torch.clamp((reward + self.gamma * (~done_mask * V_S_prime).view(-1,1)/ reward.shape[1] - V_S/ reward.shape[1]).detach(), -1, 1) # (B,)

        # Compute critic loss
        loss_func = nn.MSELoss(reduce=False)
        value_loss = (omiga * (loss_func(V_S.repeat(1, reward.shape[1]), Q_S))).sum(-1).mean()
