...
class A2C_WA_model_loss2(BaseRLAgent):
    @staticmethod
    ...
    def action_before_train(self):
        ...
        while not self.facade.is_training_available:
            ...
            prepare_step += 1
        # training records
        self.training_history = {"critic_loss": [], "actor_loss": [], 'entropy_loss':[], 'advantage':[],'corr':[],'corr_abs':[],'simi':[]}
...
#         done_mask = done_mask.to(float)
        ...
        critic_loss, actor_loss, entropy_loss, advantage,simi,corr,corr_abs = self.get_a2c_loss(observation, policy_output, reward, done_mask, next_observation)
        ...
        self.training_history['simi'].append(simi.item())
        self.training_history['corr'].append(corr.item())
        self.training_history['corr_abs'].append(corr_abs.item())
        ...
        return {"step_loss": (self.training_history['actor_loss'][-1], 
                              ...
                              self.training_history['corr'][-1],
                              self.training_history['corr_abs'][-1],
                              self.training_history['simi'][-1])}
...
#         V_S_prime = self.critic({'state_emb': S_prime})['v'].detach()
        ...
        omiga_ = (self.alpha * reward + (1-self.alpha)) / (self.alpha * torch.sum(reward,dim=-1).view(-1,1) + (1-self.alpha)* reward.shape[1]).detach()
...
#         critic_reg = current_critic_output['reg']
        ...
        advantage_ = torch.clamp((reward + self.gamma *omiga_* (~done_mask * V_S_prime).view(-1,1) - V_S/ reward.shape[1]).detach(), -1, 1) # (B,)
        
        advantage = torch.clamp((reward + self.gamma *omiga* (~done_mask * V_S_prime).view(-1,1) - V_S.detach()/ reward.shape[1]), -1, 1)
...
#         actor_loss = torch.mean(torch.sum(logp * (advantage.view(-1,1) + self.advantage_bias), dim = 1))
        ...
        weight_loss = -torch.mean(torch.sum(logp_ * advantage, dim=1 ))
...
#         actor_reg = policy_output['reg']
        ...
        simi = F.cosine_similarity(omiga.detach(),omiga_, dim=1)
        
        data_ = torch.stack((omiga.T.detach(),omiga_.T), dim=1)
        corr_matrix = torch.corrcoef(data_.view(-1, data_.size(-1)))
        corr = torch.diag(corr_matrix[0::2,1::2])
        
        
        
        return value_loss, actor_loss, entropy_loss, torch.mean(advantage),torch.mean(simi),torch.mean(corr),torch.mean(torch.abs(corr))
