...
class A2C_WAWC_model_loss(BaseRLAgent):
    @staticmethod
    def parse_model_args(parser):
        ...
        parser.add_argument('--weight_lr', type=float, default=1e-4, 
                            help='decay rate for critic')
        parser.add_argument('--weight2_lr', type=float, default=1e-4, 
                            help='decay rate for critic')
    ...
    def __init__(self, args, facade):
        ...
        self.weight2_lr = args.weight2_lr
        ...
        self.weight_model = facade.weight_model
        self.weight_optimizer = torch.optim.Adam(self.weight_model.parameters(), lr=args.weight_lr, 
                                                 weight_decay=args.critic_decay)
        
        self.weight2_model = facade.weight2_model
        self.weight2_optimizer = torch.optim.Adam(self.weight2_model.parameters(), lr=args.weight2_lr, 
                                                 weight_decay=args.critic_decay)
...
#         omiga = (self.alpha * reward + (1-self.alpha)) / (self.alpha * torch.sum(reward,dim=-1).view(-1,1) + (1-self.alpha)* reward.shape[1])
        ...
        omiga_ = self.weight_model(current_policy_output,next_policy_output,reward)['weight'].detach()
        omiga = self.weight_model(current_policy_output,next_policy_output,reward)['weight']
        
        omiga_2 = self.weight2_model(current_policy_output,next_policy_output,reward)['weight'].detach()
        omiga2 = self.weight2_model(current_policy_output,next_policy_output,reward)['weight']
#         import pdb
#         pdb.set_trace()
#         Q_S = reward + self.gamma  * (~done_mask * V_S_prime).view(-1,1)/ reward.shape[1]
        Q_S = reward.detach() + self.gamma * omiga * (~done_mask * V_S_prime.detach()).view(-1,1)
        Q_S_ = reward + self.gamma * omiga_ * (~done_mask * V_S_prime).view(-1,1)


        # Compute critic loss
       
        value_loss = F.mse_loss(V_S.squeeze(), Q_S_.sum(-1)).mean()
        
        weight_loss = F.mse_loss(V_S.detach().squeeze(), Q_S.sum(-1)).mean()
        # Regularization loss
...
#         pdb.set_trace()
        ...
        V_S = self.critic(current_policy_output)['v'].view(-1,1).detach()
            
        
        advantage_ = torch.clamp((reward + self.gamma *omiga_2* (~done_mask * V_S_prime).view(-1,1) - V_S/ reward.shape[1]).detach(), -1, 1) # (B,)
        
        advantage = torch.clamp((reward + self.gamma *omiga2* (~done_mask * V_S_prime).view(-1,1) - V_S.detach()/ reward.shape[1]), -1, 1)

        # Compute actor loss
        
        current_policy_output = self.facade.apply_policy(observation, self.actor)
        A = policy_output['action']
        A_ = policy_output['action'].detach()
        logp_ = -torch.log(torch.gather(current_policy_output['candidate_prob'],1,A_-1) + 1e-6).detach()
...
#         actor_loss = torch.mean(torch.sum(logp * (advantage.view(-1,1) + self.advantage_bias), dim = 1))
        actor_loss = torch.mean(torch.sum(logp * advantage_, dim=1 ))
        weight2_loss = -torch.mean(torch.sum(logp_ * advantage, dim=1 ))
        entropy_loss = -torch.sum(current_policy_output['candidate_prob'] \
                                  * torch.log(current_policy_output['candidate_prob'] + 1e-6), dim = 1).mean()
...
#         actor_reg = policy_output['reg']
        ...
        if do_actor_update and self.actor_lr > 0:
            # Optimize the actor             self.actor_optimizer.zero_grad()
            self.weight2_optimizer.zero_grad()
            self.actor_optimizer.zero_grad()
            (actor_loss + self.entropy_coef * entropy_loss).backward()
            weight2_loss.backward()
            self.actor_optimizer.step()
            self.weight2_optimizer.step()
    ...
    def save(self):
        ...
        torch.save(self.weight_model.state_dict(), self.save_path + "_weight_model")
        torch.save(self.weight_optimizer.state_dict(), self.save_path + "_weight_optimizer")
        
        torch.save(self.weight2_model.state_dict(), self.save_path + "_weight2_model")
        torch.save(self.weight2_optimizer.state_dict(), self.save_path + "_weight2_optimizer")
    ...
    def load(self):
        ...
        self.weight_model.load_state_dict(torch.load(self.save_path + "_weight_model", map_location=self.device))
        self.weight_optimizer.load_state_dict(torch.load(self.save_path + "_weight_optimizer", map_location=self.device))
        
        self.weight2_model.load_state_dict(torch.load(self.save_path + "_weight2_model", map_location=self.device))
        self.weight2_optimizer.load_state_dict(torch.load(self.save_path + "_weight2_optimizer", map_location=self.device))
