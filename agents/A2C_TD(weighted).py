...
#         V_S_prime = self.critic({'state_emb': S_prime})['v'].detach()
        import pdb
        pdb.set_trace()
        omiga = (self.alpha * reward + (1-self.alpha)) / (self.alpha * reward + (1-self.alpha)* reward.shpae[1])
        Q_S = reward + self.gamma * omiga * (done_mask * V_S_prime).view(-1,1)
...
#         actor_loss = torch.mean(torch.sum(logp * (advantage.view(-1,1) + self.advantage_bias), dim = 1))
        actor_loss = torch.mean((logp.view(-1,1) * (advantage.view(-1,1) + self.advantage_bias)).squeeze())
