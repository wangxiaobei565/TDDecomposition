...
#         done_mask = torch.FloatTensor(done_mask)
    ...
    def get_slateq_loss(self, observation, policy_output, reward, done_mask, next_observation, 
        ...
        current_Q = torch.max(current_Q, dim = 1)[0].view(-1,1)
        ...
        target_Q = (torch.mean(reward, dim = 1) + self.gamma * (done_mask.view(-1) * torch.max(target_Q, dim = 1)[0])).view(-1,1)
