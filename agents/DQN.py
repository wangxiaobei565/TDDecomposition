...
#         done_mask = torch.tensor(done_mask)
        ...
        Q_loss = self.get_dqn_loss(observation, policy_output, reward, done_mask, next_observation)
    ...
    def get_dqn_loss(self, observation, policy_output, reward, done_mask, next_observation, 
        ...
        target_Q = next_policy_output['action_Q'].sum(dim=1)
        
        target_Q = reward + self.gamma * (~done_mask * target_Q).detach()

        # Compute critic loss
        
        Q_loss = F.mse_loss(current_Q, target_Q).mean()
