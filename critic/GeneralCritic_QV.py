...
class GeneralCritic_QV(nn.Module):
    @staticmethod
...
#         reg = get_regularization(self.net, self.state_encoder)
        reg_Q = get_regularization(self.net)
    
        V = self.net2(state_emb).view(-1)
        reg_V = get_regularization(self.net2)
        
        return {'q': Q, 'reg_q': reg_Q, 'v': V, 'reg_v': reg_V}
