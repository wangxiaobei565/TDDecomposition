...
class OneStageFacade_HAC_de():
    ...
    def __init__(self, args, environment, actor, critic1, critic2):
        super().__init__()
...
#             "next_history_features": torch.zeros(self.buffer_size, self.env.reader.max_seq_len, self.env.reader.item_vec_size),
            ...
            "hac_pro": torch.zeros(self.buffer_size),   # item filtering network as hyper-action
            ...
            "action_features":torch.zeros(self.buffer_size, self.slate_size, self.candidate_features.shape[1]),
...
#         feed_dict = utils.wrap_batch(observation, device = self.device)
        ...
        feed_dict['candidate_features'] = self.candidate_features.unsqueeze(0)
...
#                 self.noise_var -= self.noise_decay
            ...
            hac_pro = torch.exp(-((ac_clamp - mean) ** 2) / (2 * std)) / (torch.sqrt(2 * torch.tensor(torch.pi) * std )) + 1e-20
            hac_pro = hac_pro.prod(dim=1)
            
#             hac_pro = -((ac_clamp - mean) ** 2) / (2 * std ** 2) - 0.5 * torch.log(torch.tensor(2 * torch.pi * (std ** 2))) 
#             hac_pro = torch.sum(hac_pro,dim=1)
...
#         pdb.set_trace()
        ...
        out_dict['list_emb'] = out_dict['item_emb'].squeeze()[action-1]
...
#         pdb.set_trace()
    ...
    def sample_buffer(self, batch_size):
        ...
        U, H, N, S, HA, A, AF, R, F, D, HP = self.read_buffer(indices)
        observation = {"user_profile": U, "history_features": H}
        policy_output = {"state_emb": S, "action_emb": HA, "action": A, "action_features": AF,"hac_pro":HP}
        reward = R
        done_mask = D
...
#         return behavior_loss
    ...
    def update_buffer(self, observation, policy_output, reward, done_mask, next_observation, info):
        ...
        - (A)ction: list of ids
        - (AF)eatures: features of (list of ids)
        ...
        self.buffer["action_features"][indices] = policy_output['action_features']
        self.buffer["action_emb"][indices] = policy_output['action_emb']
        self.buffer["hac_pro"][indices] = policy_output['hac_pro']
...
#         N = self.buffer["next_history_features"][indices]
        ...
        AF = self.buffer["action_features"][indices]
        R = self.buffer["reward"][indices]
        F = self.buffer["feedback"][indices]
        D = self.buffer["done"][indices]
        HP = self.buffer["hac_pro"][indices]
        return U, H, N, S, HA, A, AF, R, F, D, HP