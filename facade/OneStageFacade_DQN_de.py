...
class OneStageFacade_DQN_de():
    ...
    def __init__(self, args, environment, Q_net, critic1):
        super().__init__()
...
#             "next_history_features": torch.zeros(self.buffer_size, self.env.reader.max_seq_len, self.env.reader.item_vec_size),
            "state_emb": torch.zeros(self.buffer_size, self.Q_net.state_dim),
            "hac_pro": torch.zeros(self.buffer_size),
            "action_emb": torch.zeros(self.buffer_size, self.Q_net.action_dim),   # item filtering network as hyper-action
...
#             hac_pro = torch.sum(hac_pro,dim=1)
        ...
        # action prob (B,L)
        action_Q = policy_model.score(out_dict['action_emb'], 
                                         out_dict['candidate_features'], 
                                         do_softmax = do_softmax)

        # two types of greedy selection
        
        _, indices = torch.topk(action_Q, k = self.slate_size, dim = 1)
        # topk action
        if batch_wise:
        ...
        # (B,K)
        out_dict['action_Q'] = torch.gather(action_Q, 1, indices) 
        # (B,L)
        out_dict['candidate_Q'] = action_Q
        return out_dict
    ...
    def sample_buffer(self, batch_size):
        ...
        U, H, N, S, HA, A, R, F, D, HP = self.read_buffer(indices)
        observation = {"user_profile": U, "history_features": H}
        policy_output = {"state_emb": S, "action_emb": HA, "action": A,"hac_pro":HP}
        reward = R
        done_mask = D
