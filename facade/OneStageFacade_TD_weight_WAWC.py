...
class OneStageFacade_TD_weight_WAWC():
    '''
    ...
    def __init__(self, args, environment, actor, critic, weight_model, weight2_model):
        super().__init__()
...
#             "next_history_features": torch.zeros(self.buffer_size, self.env.reader.max_seq_len, self.env.reader.item_vec_size),
            ...
            "reward": torch.zeros(self.buffer_size, self.slate_size),
...
#         pdb.set_trace()
    ...
    def sample_buffer(self, batch_size):
        ...
        U, H, N, S, HA, A, AF, R, F, D = self.read_buffer(indices)
        observation = {"user_profile": U, "history_features": H}
        policy_output = {"state_emb": S, "action_emb": HA, "action": A, "action_features": AF}
        reward = R
        done_mask = D
