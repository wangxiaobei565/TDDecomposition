...
#         return (np.mean(self.hist_Q) + self.q_laplace_smoothness) / (new_q + self.q_laplace_smoothness)
    ...
    def apply_policy(self, observation, policy_model, epsilon = 0, 
                     do_explore = False, do_softmax = True,return_log_prob=True):
        '''
        @input:
...
#                 self.noise_var -= self.noise_decay
        ...
        if return_log_prob:
            # calculate log_prob of action
            chosen_action_prob = torch.gather(action_prob, 1, indices)
            log_prob = torch.log(chosen_action_prob + 1e-10) # add small number to avoid log(0)
