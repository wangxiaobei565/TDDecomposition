...
mkdir -p output/ml1m/agents/11TD_decomposition/2de_reqdqn/
...
for MAX_STEP in 20 
do
    for Q_LR in 0.000003 0.00003 
    do
#         for SCORER in "WideDeep"
        for SCORER in "DotScore"
        do
            for CRITIC_LR in 0.000001 0.000003 0.00003
            do
                for SEED in 11 13 17 19 23 
                do
                    mkdir -p ${output_path}agents/11TD_decomposition/2de_reqdqn/dqn_${SCORER}_Q${Q_LR}_V${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/

                    python train_dqn_de.py\
                        --env_class KREnvironment_GPU\
                        --Q_net_class OneStagePolicy_with_DotScore\
                        --critic_class1 GeneralCritic_V\
                        --agent_class DQN_de\
                        --facade_class OneStageFacade_DQN_de\
                        --seed ${SEED}\
                        --cuda 0\
                        ...
                        --save_path ${output_path}agents/11TD_decomposition/2de_reqdqn/dqn_${SCORER}_Q${Q_LR}_V${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/model\
                        ...
                        --check_episode 10\
                        --topk_rate ${TOPK}\
                        > ${output_path}agents/11TD_decomposition/2de_reqdqn/dqn_${SCORER}_Q${Q_LR}_V${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/log
                done
            done
        done
    done
