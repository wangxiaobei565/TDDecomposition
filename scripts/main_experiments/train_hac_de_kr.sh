...
mkdir -p output/kr/agents/1de_reqhac/
...
for MAX_STEP in 20 
do
    for CRITIC_LR in 0.000001 0.000003 0.00001
    do
...
#         for SCORER in "DotScore"
        do
            ...
            do
                for ACTOR_LR in 0.00001 0.00003 0.0001
                do
                    mkdir -p ${output_path}agents/1de_reqhac/de_hac_${SCORER}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/

                    python train_ddpg_de.py\
                        --env_class KREnvironment_GPU\
                        --policy_class OneStagePolicy_with_${SCORER}\
                        --critic_class1 HACCritic\
                        --critic_class2 GeneralCritic_V\
                        ...
                        --save_path ${output_path}agents/1de_reqhac/de_hac_${SCORER}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/model\
                        ...
                        --check_episode 10\
                        --topk_rate ${TOPK}\
                        > ${output_path}agents/1de_reqhac/de_hac_${SCORER}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/log
                done
            done
        done
    done
