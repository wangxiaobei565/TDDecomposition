...
mkdir -p output/ml1m/agents/11TD_decomposition/1reqhac/
...
for MAX_STEP in 20 
do
#     for CRITIC_LR in 0.00001 0.00003 0.0001 0.0003 
    for CRITIC_LR in 0.000001 
    do
#         for SCORER in "WideDeep"
        for SCORER in "DotScore"
        do
            for SEED in 13 17 19 23 11 
            do
#                 for ACTOR_LR in 0.000001 0.000003 0.00001 0.00003
                for ACTOR_LR in 0.000003
                do
                    mkdir -p ${output_path}agents/11TD_decomposition/1reqhac/A2C_${SCORER}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/

                    python train_ddpg.py\
                        --env_class ML1MEnvironment_GPU\
                        --policy_class OneStagePolicy_with_${SCORER}\
                        --critic_class HACCritic\
                        --agent_class HAC_effect\
                        ...
                        --save_path ${output_path}agents/11TD_decomposition/1reqhac/A2C_${SCORER}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/model\
                        ...
                        --check_episode 10\
                        --topk_rate ${TOPK}\
                        > ${output_path}agents/11TD_decomposition/1reqhac/A2C_${SCORER}_actor${ACTOR_LR}_critic${CRITIC_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/log
                done
            done
        done
    done
