...
mkdir -p output/kr/agents/
mkdir -p output/kr/agents/1TD_others/de_reqhac_var/
...
#         for SCORER in "WideDeep"
        ...
        do
            for NOISE in 0.9 0.7 0.5 0.3 #1 0.1 0.01 0.001 0.0001 0.00001 0.000001 0.0000001
            do
#                 for ACTOR_LR in 0.00003
                for SEED in 11 13 17 19 23
                do
                    mkdir -p ${output_path}agents/1TD_others/de_reqhac_var/de_hac_${SCORER}_lambda${LAMBDA}_actor${ACTOR_LR}_critic${CRITIC1_LR}_${CRITIC2_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/
                    ...
                    python train_ddpg_de.py\
                        ...
                        --save_path ${output_path}agents/1TD_others/de_reqhac_var/de_hac_${SCORER}_lambda${LAMBDA}_actor${ACTOR_LR}_critic${CRITIC1_LR}_${CRITIC2_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/model\
                        ...
                        --check_episode 10\
                        --topk_rate ${TOPK}\
                        > ${output_path}agents/1TD_others/de_reqhac_var/de_hac_${SCORER}_lambda${LAMBDA}_actor${ACTOR_LR}_critic${CRITIC1_LR}_${CRITIC2_LR}_niter${N_ITER}_reg${REG}_ep${INITEP}_noise${NOISE}_bs${BS}_epbs${EP_BS}_step${MAX_STEP}_seed${SEED}/log
                done
            done
        done
    done
